...
public class Exercises {
	...
	public List<String> array2List(String[] stringArray) {
		...
		for (int i = 0; i <stringArray.length; i++) {  
			...
			result.add(stringArray[i]);      //taking the value of each element through the loop and adding it to result
											//can also do   String s = stringArray[1];
		}
		return result;
	}
	...
	public String[] list2Array(List<String> stringList) {
		...
		for (String listOfStrings : arrayString) 
		{
	...
	public List<Double> arrayInt2ListDouble(int[] intArray) {
		...
		for(int variable: intArray) {
			arrayInt2ListDouble.add(variable/2.0);
		}
	...
	public Integer findLargest(List<Integer> integerList) {
		...
		for(int i = 0; i < integerList.size(); i++){
	        if(integerList.get(i) > variable){
	            variable = integerList.get(i);
	       }
	    }
		return variable;
	}
	...
	public List<Integer> oddOnly(Integer[] integerArray) {
		...
		for(int variable : integerArray) {
			if(variable % 2 != 0) {
				oddOnly.add(variable);
			}
		}
		return oddOnly;
	}
	...
	public boolean foundIntTwice(List<Integer> integerList, int intToFind) {
		...
		//List<Integer> foundIntTwice = new ArrayList<Integer>(); NOT NEEDED
		
		boolean foundIntTwice = false;
		
		Integer variable = null;
				
		for(Integer listOfElements : integerList) {
			if(listOfElements == variable) {
				foundIntTwice = true;
			}
			if(listOfElements == intToFind) {
				variable = listOfElements;
			}
		}
    ...
    ** INTERVIEW QUESTION **	 
	...
	public List<String> fizzBuzzList(Integer[] integerArray) {
		...
		for(int i = 0; i < integerArray.length; i++) {
			if(integerArray[i] % 3 == 0 && integerArray[i] % 5 == 0) {
				...
				fizzBuzzList.add(String.valueOf(integerArray[i]));
			}
		}
			return fizzBuzzList;
	}
	...
	public List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {
		...
		Integer longerList = null;  

		if (listOne.size() >= listTwo.size()) {
			longerList = listOne.size();
	    }
		else {
			longerList = listTwo.size();
		}
	...
	public List<Integer> boardingGate(List<Integer> seatNumberList) {
		...
		List<Integer> boardingOrderList = new ArrayList<Integer>();
		
		Queue<Integer> boardingOrderOne = new LinkedList<Integer>();
		Queue<Integer> boardingOrderTwo = new LinkedList<Integer>();
		Queue<Integer> boardingOrderThree = new LinkedList<Integer>();

		for (int i = 0; i < seatNumberList.size(); i++) 
		{
			if (seatNumberList.get(i) <= 10 && seatNumberList.get(i) >= 1) 
			{
				boardingOrderOne.offer(seatNumberList.get(i));
			}
			else if (seatNumberList.get(i) <= 20 && seatNumberList.get(i) >= 11) 
			{
			    boardingOrderTwo.offer(seatNumberList.get(i));
			}
			else if (seatNumberList.get(i) <= 30 && seatNumberList.get(i) >= 21) 
			{
			    boardingOrderThree.offer(seatNumberList.get(i));
			}
		}
			    
		for (int num : boardingOrderOne) 
		{
			boardingOrderList.add(num);
		}
		for (int num : boardingOrderTwo) 
		{
			boardingOrderList.add(num);
		}
		for (int num : boardingOrderThree) 
		{
			boardingOrderList.add(num);
		}
		return boardingOrderList;
	}
}
