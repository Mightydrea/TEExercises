...
SELECT category.name AS film_type
FROM film
...
SELECT category.name AS film_type, film.rating
FROM film
JOIN film_category ON film.film_id = film_category.film_id
JOIN category ON film_category.category_id = category.category_id
...
SELECT category.name AS film_type, rating, length
FROM film
JOIN film_category ON film.film_id = film_category.film_id
JOIN category ON film_category.category_id = category.category_id
...
--will need a sum function 

SELECT first_name, last_name, SUM(payment.amount) as dollars_spent
FROM customer
...
SELECT store.store_id, address.address, COUNT(rental.rental_id) AS total_rentals, SUM(payment.amount) total_sales, AVG(payment.amount) AS average_sales
FROM store
...
SELECT film.title AS movie_title, COUNT(rental_id) AS total_rentals
FROM film
...
SELECT category.name, COUNT(category.name) AS total_number_rented
FROM category
...
ORDER BY total_number_rented DESC LIMIT 5;

-- 18. The top five Action film titles by number of rentals 
-- (#1 should have 30 rentals and #5 should have 28 rentals)

SELECT film.title AS movie_name, COUNT(category.name) AS total_number_rented
FROM category
JOIN film_category ON category.category_id = film_category.category_id
JOIN film ON film_category.film_id = film.film_id
JOIN inventory ON film.film_id = inventory.film_id
...
SELECT actor.first_name ||' '|| actor.last_name AS actor_name, COUNT(rental_id) AS number_of_rentals
FROM actor
