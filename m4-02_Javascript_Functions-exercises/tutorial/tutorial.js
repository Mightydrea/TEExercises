 ...
 */
function returnGivenParameter(value){
  return value;
}
 ...
 */
function takeOptionalParameter(value = 0){
    return value;
}
...
function filterStringArrayForSon(arrayToFilter) {
  return arrayToFilter.filter(
    (value) => { return value.includes('son'); }
    );
}
...
function makeNamesAllCaps(arrayToCapitalize) {
  return arrayToCapitalize.map(
    (arrayToCapitalize) => {return arrayToCapitalize.toUpperCase()}
  );
}
 ...
 * @returns {number]} convert to string
 */
